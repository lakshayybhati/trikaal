...
- **G-§8.C.3 (external validation):** Cell 1 (BSQ + OHLCV), universe-trained, within **~10–15% RankIC of published Kronos-small** on a common slice. If Cell 1 is materially off, our BSQ baseline is broken and every comparison is suspect → fix before trusting the ablation. [inv. 4]
