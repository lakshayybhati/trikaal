...
4. **The 2×2 ablation is the proof:** {BSQ, FSQ} × {OHLCV-only, +microstructure} at matched params/bits. Cell 1 (BSQ + OHLCV-only) is a faithful **Kronos_small** reproduction — keep the AR backbone *exactly* Kronos_small (8 layers, d_model 512, d_ff 1024, 8 heads) so the comparison stays clean.
...
7. **Determinism is a deliverable, not hygiene.** Every run must be reproducible from one config file + a pinned seed; every produced dataset is content-hashed so any past prediction can be reconstructed.
