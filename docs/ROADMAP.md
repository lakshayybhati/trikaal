...
- **M4b (LOCAL, parallel + raw-cache)** ◑ — **machinery built, reviewed, and run on a bounded REAL subset; full 200-pair lake is a bandwidth-bound continuation.** Bootstrapped the survivorship-correct universe FROM THE DUMP INDEX (`universe_bootstrap.py`: S3 scan → 200 USDT perps incl. **41 delisted**, listing/delisting from dump coverage, volume-rank proxy) → `config/universe_full.yaml` + corpus-purpose metadata. Governed producer–consumer (`ingest_orchestrator.py`): pooled SHA-256-verified downloads fill a **bounded raw cache**, each symbol reduced **in-memory** → feature lake → manifest-recorded → **raw EVICTED (evict-after-record, single chokepoint, ledger commit marker)**; resumable, per-symbol failure-isolated. **Decisive measurement: the link is the wall — 8.3 MB/s aggregate, parallelism does NOT beat it** ⇒ full plan ≈ **445 GB / ~15 h / ~315 M bars** (315 M flagged: below the 500 M–1 B band but adequate — past saturation for a 27 M model; the lake is eval/regime breadth, not training fuel — `[[training-saturation-budget]]`). Reproducibility anchor = content-hashed **lake**; raw checksums **provenance-only** (survives eviction). **14-agent adversarial verification workflow** over the M4b surface found + fixed **2 HIGH** (dump-publish-lag fabricated delistings → survivorship corruption, now lag-tolerant; orchestrator failure-leg disk-leak/counter-phantom) **+ 1 MED** (interior-gap silent symbol drop → plan now from real dump months); evict-after-record invariant re-verified clean. Post-review hardening: the in-line real-bar causal sweep is now a **hard gate** (RED → `FatalIngestError` aborts non-zero before record/evict) **broadened to a live + a delisting-tail symbol**, plus an orchestrator-path per-symbol-independence test. **159 tests green, ruff clean.** Realized 4-symbol lake: **3.1M bars, KLAYUSDT spans all 4 regimes (2021-10→2024-10)**, eviction + resume proven on real data, cross-section causal sweep GREEN, universe Merkle `f0ca2d37…`. Write-up: `docs/milestone4b_universe_ingest.md`.
- **Exit gate:** 100–200 USDT pairs ingested (SHA-256, content-hashed manifest, **listing + delisting dates recorded**), full Parquet/DuckDB lake, causal exhaustive sweep green on a sampled cross-section of real universe bars. — *machinery + config + manifest + cross-section sweep ✅; full-200 lake pending the ~15 h bandwidth-bound run (one resumable command).*
...
- **Result (Gate A met, LOCAL on the M3 checkpoint):** 9 reusable `trikaal/eval/*` modules (metrics/costs/strategy/dsr/diagnostics/folds/placebo/predict/harness + the M2 `ic_screen`) + `tokenizer.decode_tokens`. **Every metric passes a known-answer test (46 eval tests green)**; the full machine runs end-to-end on the real BTCUSDT lake + M3 checkpoint at h∈{5,15,60} (folds → rollout → strategy → net-IR@0.30% + cost-stress + break-even → DSR/PBO → Cell-5 placebo → diagnostics → synthetic Q3/Q4 path), content-hashed; the **§8.A.5 causal exhaustive sweep PASSES on a Q4 sample (420/420 bars, leak-free + has teeth)**. A 7-agent adversarial review caught + fixed a **HIGH** lookahead (rollout μ̂ off-by-one → excluded the t→t+1 move so μ̂ matches the `log(C_{t+h}/C_{t+1})` label) and a cost-`σ̄` future-mean (→ causal). **Numbers are meaningless** (dev-grade, one symbol, synthetic Q3/Q4). Kronos metric cross-check deferred to M6 (weights load there anyway). Write-up: `docs/milestone5_eval_harness.md`.
