...
| A2 | **The embargo is justified on the SIGNED-return channel only, and the receipt records `premise_supported_for_ABS_returns = False`.** Signed ACF at lag 60 is 0.0072 mean on the **pinned 40** (0.0067 across all 200), a ≈24× margin. **\|return\| ACF is 0.2323 at lag 60 on the pinned 40** — 0.2097 across all 200 — and **0.3604 on the worst single symbol**; at lag 240 it is 0.1837 on the pinned 40 and 0.1597 across all 200. **[CORRECTED 2026-08-20. This row previously read "0.20 at lag 60 and 0.15 at lag 240", and the error was not arithmetic but ASYMMETRIC: the signed figure was quoted across BOTH bases, where the margin is comfortable, while \|return\| was quoted on whichever basis gave the smaller number — and 0.1597 was rounded DOWN to 0.15. Both understatements pointed the same way, toward a milder limitation, and the worst-symbol 0.3604 appeared on no reader-facing surface at all. Every figure now names its basis.]** *"A leakage channel that isn't signed-return autocorrelation wouldn't be caught by it."* The end-to-end alternative was costed at 3× the run and not funded. | `m6_c20_embargo_premise.json` |
...
| C9 | **`batch_size` is a bare dataclass default — `src/trikaal/train/orchestrator.py:90`, `batch_size: int = 32`, and NO `PINNED_*BATCH*` constant exists anywhere in `src/`, `tests/` or `scripts/`.** It is the **third multiplicand** of the compute-optimal figure the paper quotes: 26,003 × 32 × 512 = 426,033,152 tokens = 20.00 tok/param. | **DISCLOSURE, NOT CLOSURE — and it is an UNSWEPT SITE OF A CLASS THIS DATACLASS HAS ALREADY HAD FIXED TWICE.** C-6 pinned `seeds` and `seq_len` here; C-18 pinned `steps_stage1/2` here. **C-18's own comment, at `orchestrator.py:153-155`, describes `batch_size`'s present state verbatim:** *"the step budget was a bare dataclass default. The money driver RECORDED it in its manifest, but **recording is not pinning** — a different budget would have run and been faithfully reported as the recipe."* Swap `steps_stage1` for `batch_size` and the sentence is still true, 64 lines above where it is written. **WHY IT DOES NOT BIAS THE VERDICT:** it is ONE SHARED FIELD across all five cells, so a drift moves every arm together and **cannot move ΔIR(4−5) in either direction** — it changes what was trained, never the paired contrast. **WHAT IT DOES THREATEN is the paper's 20.00 tok/param claim and reproducibility**, not the ablation. It IS recorded (`scripts/m6_money_run.py:143`) and `tests/run/test_money_driver.py:397` asserts the recording — **and recording is exactly what C-18 ruled insufficient**, so this row is a disclosure and not a closure. **v2 FIX, one line:** `PINNED_MONEY_BATCH_SIZE` + a `__post_init__` assertion on the money path + a mutation KAT — **deliberately deferred past the run under the remediation rule**, because new code written fast against a frozen instrument is the least-reviewed code in the project and the run is unblocked. |
...
| # | item |
|---|---|
| F1 | **`image` and `lockfile_sha256` were missing from `PROVENANCE_IDENTITY_KEYS`.** Two boxes can carry identical torch/numpy/driver strings and still differ in container image or resolved dependency set. Added; all 13 keys mutation-proven to refuse. Non-blocking under the bar (it *strengthens* a refusal), recorded because the gap existed while fan-out was already planned. **[13 was the count at v1.6.23 and this row is left as written. The set later grew to the 16 of `PROVENANCE_IDENTITY_KEYS` — v1.6.25 added `git_commit`, `steps_stage1`, `steps_stage2`, and `platform` was subsequently split into `platform_abi`. G4 below says 16 for that reason; neither number is wrong at its own date.]** |
...
| **G7** | **The image is ~7 GB and we throw its torch away.** `pytorch/pytorch:2.5.1-cuda12.4-cudnn9-devel` ships torch 2.5.1; we pip-install 2.12.1+cu130 on top. Setup is dominated by that pull, and three of five boxes on 2026-08-04 stalled in `loading` on it. A slim CUDA base is ~2 GB and would attack the measured dominant term directly. | **REGISTERED, NOT ACTED ON — supervisor's ruling, and the evidence supports it: the same 7 GB image provisioned in 29 s and stalled at 908 s the SAME DAY, so the stalls point at the host, not the image.** The trade is ~5 GB × 5 boxes ≈ $0.20 of transfer against **an untested path AND an identity-key change on a $78–108 run** — not worth taking. My reasoning is kept intact here so it is available if the fan-out proves slow in practice: the test would be one box, one variable, `nvidia/cuda:*-runtime` + `apt install python3-pip`, timed against the 0.28 GPU-h baseline. |
| **G6** | **`money_run_manifest.json`'s RUN-LEVEL provenance stamps `attention_mode: "unknown"`** while every eval ARTIFACT under the same run stamps `sdpa_deterministic`. Found in the P1 receipt. | **Deferred, and the reason is precise: the refusal path reads the ARTIFACTS, not the run manifest.** `provenance_failures` compares per-unit artifact stamps, so the identity surface is intact and the fan-out refusal is unaffected — the run manifest's copy is a receipt field that is less informative than it looks. Non-blocking under the bar (moves no number, cannot reach the verdict). Recorded rather than fixed because this pass is closed to anything not named. |
