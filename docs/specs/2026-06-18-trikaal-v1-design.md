...
> **A microstructure-aware FSQ tokenizer for financial K-lines** — two mechanically-independent legs, one claim: (1) **Finite Scalar Quantization (FSQ)** replaces Kronos's Binary Spherical Quantization (no auxiliary commitment loss, no codebook-collapse failure mode, bounded quantization error); (2) the per-bar input vector carries **free microstructure** — **trade-flow imbalance (TFI) and the aggTrades trade-flow statistics** — beyond OHLCV. **Funding and open interest are SPECIFIED and wired (dims 13–15, with a tripwire that fails loudly if they ever activate) but are STRUCTURALLY ABSENT in the v1 lake: constant-zero and masked on 100% of 304,625,181 bars**, for the documented OI-retention reason (`docs/milestone4b_universe_ingest.md:129`). **The input is 16-wide with 13 live dims.** This makes *free* literally true: the aggTrades dumps are free, funding/OI need the futures API. §7 v1.6.17; measured in `runs_manifest/m6_c15b_lake_surface_check.json`.
...
`ruff` (lint) + `ruff format` (formatter) configured in `[tool.ruff]`. Enabled rule sets: `E,F,W` (pyflakes/pycodestyle), `I` (import sort), `B` (bugbear), `UP` (pyupgrade), `NPY` (numpy), `PD` (pandas-vet on `data/`), `RUF`. `line-length = 100`. The one project-specific custom lint we enforce by `flake8`-style grep-test in CI: **no bare `log(` / division without an `eps`** in `data/` (stability rule, feature-spec §5) — implemented as a `tests/data/test_no_unsafe_ops.py` AST check. **[NOT IMPLEMENTED — annotated 2026-08-20. That file is not tracked and is referenced nowhere else; the custom lint described here does not run. Measured before relabelling: a first-cut AST check of this shape raises 59 candidate findings across `src/trikaal/data/`, nearly all of them sites where the divisor is provably non-zero by another invariant — so the check needs per-site adjudication to mean anything, and a version weakened until it passes would be decoration. Recorded as unbuilt rather than written badly.]**
