...
\begin{figure}[t]
  ...
  the pre-registered acceptance rule over the three calibration seeds is mean $\geq 0.90$ and every
  seed $\geq 0.85$, so both thresholds are drawn. The original contextual design, the pointwise
  ...
  the same probe applied to real microstructure by the standing gate, which runs after first-stage
  training and before any second-stage expenditure. \PH{no values drawn} --- the panel shows six empty
  slots rather than placeholder bars, because bars near the gate would imply a passing result
  nobody has measured. Six numbers drop in from the gate's own receipt, and the gate halts the run
  if any of them falls below the rule.}
