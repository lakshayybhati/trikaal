...
% artifact: git show a1729b8:runs_manifest/m6_interface_respec_design_pass.json :: gate2_blocked_diagnosis.capacity_arithmetic.{sign_acc_0.9_requires_rho=0.951, fine_bits_available=10.26, total_bpt_pinned=20.058}
...
% 'capacity' at ANY of its four revisions (a1729b8, e1feb64, d9e1b53, 39be8ba) and its main() writes
...
weakly covariant with OHLCV shape --- they carry information about the near future precisely
...
\textbf{One point of order before the measurement, because this section is easy to read as
describing the tokenizer introduced at the start of \cref{sec:mechanism}.} The reconstruction
results below were produced by a tokenizer that already carries interventions (i) and (ii) of
\cref{sec:mech-fix} --- the per-bar fine encoder and the bottleneck leg --- which are not
introduced until three pages from here. They are present because the eviction they expose survives
them; the original contextual design fails the same test more severely, not less.
...
the fixture is thirteen dimensions wide, matching the live width of the real vector, and stands to
it as follows: one return dimension for price, eleven mutually dependent \emph{fillers} standing
for the OHLCV block and for the microstructure channels that co-vary with it, and one independent
\emph{signal} dimension standing for the channel whose predictive content is its own. The twelve
that carry the comparison are marginally standard normal --- so all twelve
contribute equally to a variance-weighted loss --- and then make eleven of them mutually dependent
while one, the \emph{signal} dimension, is independent of everything. A thirteenth, the
\emph{return} dimension, is deliberately \emph{not} standard normal: it carries $3.15\times$ the
marginal standard deviation of the rest and stands in for price. Matching the twelve is what
isolates dependence from variance; giving the return a larger scale is what supplies the
high-variance channel the mechanism is about.
...
\begin{figure}[t]
  ...
  \caption{\textbf{Per-bar recoverability sorts by variance class, not by task relevance, and
...
% artifact: git show a1729b8:runs_manifest/m6_interface_respec_design_pass.json :: gate2_blocked_diagnosis.three_seed_winner_receipt (index 9: 0.91 / 0.26 / 0.10; note: tiny shells d64, 600 steps)
...
\begin{figure}[t]
  ...
  feature space.} \emph{Plotted:} the forecast--state probe correlation against training step, with
  the dashed rule in each panel marking the pre-registered detection threshold of $0.30$. The nats
  quoted below each panel are the planted and recovered information for the same runs, read from
  their receipts rather than from the curve. \textbf{Left:} $1.151$ nats planted in feature space,
  where it must survive tokenization --- the probe never leaves zero, staying below $0.05$ at all
  $40$ evaluation points. \textbf{Centre:} $0.9003$ nats planted directly in token space,
...
the objective a term that can. We make three changes, and we state precisely what the evidence
about them supports. They were introduced \emph{cumulatively} --- (i), then (i)+(ii), then
(i)+(ii)+(iii) --- so the ladder establishes that no \emph{prefix} of the three suffices and that
the full triple does. It does not establish that (i) or (ii) is necessary, because neither was
removed from the final triple; only (iii) has that evidence, being inert at every $\lambda$ without
the others. \textbf{The supported claim is therefore that no prefix of the three was sufficient
and the full triple was}, and we report the intermediate failures because they are what establishes
it.
...
% artifact: git show a1729b8:runs_manifest/m6_interface_respec_design_pass.json :: gate2_blocked_diagnosis.iteration_history.iter1_pointwise_encoder_only
...
% artifact: git show a1729b8:... :: iteration_history.iter2_added_per_bar_bottleneck_leg; runs_manifest/m6_interface_respec_design_pass.json :: gates.gate2_legibility.{new_by_seed={0:0.5014,1:0.4995,2:0.5182}, threshold=0.9, pass=false, protocol}
...
$\lambda$ is calibrated, not chosen. The search ran over two knobs: the class weight $\lambda$ and
$\beta$, the weight on the bottleneck leg's own reconstruction term. Across $26$ trials no
$(\lambda, \beta)$ pair cleared $0.90$
