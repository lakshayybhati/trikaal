...
% artifact: docs/BUILD_RECORD.md §2 D20 — fixed at 0b9c804, deterministic_algorithms=True on the production path; penalty 13.175% per docs/m6_cuda_probe_report.md
...
amendment helps, we record whether it tightens or loosens the criterion. The log carries $65$
...
\textbf{That property is weaker than it sounds, and the weaker version is the one we claim.}
Loading from a receipt prevents a figure from contradicting the receipt it \emph{reads}. It does
not prevent a figure from reading the wrong receipt, or from reading no receipt at all for a panel
whose result exists --- and a generator that omits an artifact fails nothing, because there is
nothing to fail. Two figures therefore carry an explicit check on top of the loading:
\cref{fig:legibility} asserts its drawn values against the gate's stop receipt and
\cref{fig:design} asserts its parameter counts against the checkpoints, and each has been observed
to fail under a deliberately wrong constant. \textbf{The claim is thus: a figure that contradicts
the artifact it loads fails to build, and where a figure could silently omit an artifact instead,
we added a check rather than assumed the property.}
