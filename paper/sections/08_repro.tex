...
\textbf{That claim is scoped to one environment, and we can now say what happens outside it.}
Re-scoring a banked predictor on a different \texttt{torch} build and a different physical GPU,
weeks later, reproduces the scored series \emph{closely but not bit-exactly}: the set of active
periods is identical at $34{,}989$, the gross series agrees to a maximum absolute difference of
$2.59\times10^{-4}$ with a correlation of $0.99999986$, and the information ratio moves by
$3.5\times10^{-4}$. A different build is a different provenance identity by construction --- the
\texttt{torch} version and the platform ABI are both identity keys --- so this does not contradict
the bit-exactness claim, which was never asserted across identities. It bounds what the claim does
not cover, which had until now been untested: replay across environments is reproducible to about
four decimal places, not to the bit.

Four limits belong with it, and we state them rather than let the agreement figure travel alone.
It is \emph{not} bit-exactness --- the residual $2.59\times10^{-4}$ is the float32
reduction-order effect that deterministic attention is already conceded not to remove. It says
nothing about \emph{training} reproducibility, which is where the same-seed divergence of
\cref{sec:lim-determinism} lives and which this does not touch. It covers one arm and one seed, so
it is not a general claim about the pipeline. And both environments ran with deterministic
attention and deterministic algorithms forced, so it establishes nothing about the default path.
What it does establish is the half that had been argued rather than shown: the scored inference
path survives a change of \texttt{torch} version and of physical machine, weeks apart, with a
bit-identical set of active decisions.
% artifact: runs_manifest/m6_cross_environment_reproduction.json (the dedicated receipt; harvested
%   from the control arm below because a reproducibility result buried in a bias test is invisible
%   where it matters) :: agreement.*, environments.{original_money_run,reproduction}.
%   THE FOUR LIMITS: three are numbered in ★_WHAT_IT_DOES_AND_DOES_NOT_ESTABLISH — (1) not
%   bit-exactness, (2) says nothing about TRAINING, (3) one arm / one seed. The fourth is readable
%   from .environments: BOTH sides ran attention_mode "sdpa_deterministic" and
%   deterministic_algorithms true, so nothing is established about the DEFAULT path.
% artifact: runs_manifest/m6_skill_vs_bias_exact.json :: control_arm.{n_active_recomputed 34989,
%   n_active_banked 34989, active_set_identical true, max_abs_diff_gross 2.5873020400202923e-04,
%   corr_gross 0.999999857150472, ir_recomputed 1.1446252585124777, ir_banked 1.1442791248262019}.
%   torch 2.5.1+cu124 on the rented box vs 2.12.1+cu130 for the money run; different physical 4090.
...
amendment helps, we record whether it tightens or loosens the criterion. The log carries $52$
distinct revision tags.

\textbf{One of those amendments was made after the run began, and it is flagged as such wherever
it bears.} Every other entry predates the existence of any evaluation unit. The exception rescoped
a codebook-utilization gate to the quantizer the specification had scoped it to, after two units
had been evaluated. We hold it to be a restoration rather than a relaxation --- the specification
sets that target for one quantizer, calls the diagnostic a health signal rather than a failure
mode, and says the other quantizer's figure is \emph{reported}; the gate had been transcribed into
our own log without its scope. But a reader is entitled to the mechanical fact independently of our
reading, so the log records it as a loosening, records what was visible when it was made, and
records the counterfactual: had the measurement come out the other way, the same restoration would
still have been correct. One consequence is pre-committed rather than left to discretion --- the
two units evaluated at that point bear on the direction of the secondary fallback comparison, so
\textbf{any fallback claim arising from this run is reported as evaluated under a post-data
amendment}, whether or not the fallback fires.
% artifact: docs/m6_prereg.md §7 v1.6.30 — the rescope, the four specification citations, the
%   transcription defect at §7 v1.6.22(e), the leak disclosure and the pre-commitment.
...
\subsection{One identity key was deliberately relaxed, on schedule grounds}
\label{sec:repro-platform}

Every unit of the ablation is stamped with sixteen provenance keys, and the verdict is refused
outright if any key disagrees across units: a cell trained and scored on a different instrument is
a different experiment, and $\Delta\IR(4-5)$ across two instruments measures the instruments as
much as the tokenizer. One of those keys was relaxed before the run, and because a reproducibility
claim is exactly the kind of sentence that gets quoted alone, we state the exception inside the
claim rather than beside it. \textbf{The units of this study ran on hardware that is identical in
fifteen of sixteen recorded respects; the sixteenth, the host kernel release, is recorded but not
required to match, and this section says why.}

The key originally carried the full platform string, kernel included. It was split: the
ABI-bearing part --- architecture and C library --- remains an identity key and still refuses on
mismatch, while the full string, kernel included, is still stamped on every artifact as a recorded
but non-compared field. \textbf{What changed is what we refuse on, never what we record.} Fifteen
keys are untouched --- GPU model, driver, CUDA build, PyTorch, NumPy, Python, commit, image,
lockfile hash, attention mode and all three determinism flags still must match exactly.

\textbf{The motive was schedule, and we state it before the technical argument rather than after.}
The rental market exposes no kernel filter, so a box's kernel is knowable only by renting it.
Across a probe and two batches we rented fourteen machines and observed fourteen distinct kernels
with no matched pair; filtering on driver, the only axis exposed, determines nothing about the
kernel. A matched multi-machine pool was therefore not purchasable at any price we found, which
forced the entire $270.4$ GPU-hour workload onto one machine --- $11.3$ days of wall-clock. The
split makes a five-machine fan-out purchasable and takes that to $54.1$ hours. The additional
\emph{work} is about $1.1$ GPU-hours --- roughly a dollar --- because the computation is identical
and only per-machine setup multiplies. Any further difference in outlay comes from renting the five
cheapest machines rather than the single cheapest, which raises the mean hourly rate; that is a
market condition rather than a property of the change. There was accordingly no work-versus-time
trade to make: the change buys wall-clock and nothing else.

The technical argument is that a kernel patch release does not participate in GPU floating-point
computation --- with GPU model, driver, CUDA build and framework fixed and deterministic
algorithms forced, no float operation or kernel selection differs. We hold that argument to its
scope: it concerns GPU arithmetic, not a general claim that kernel versions are immaterial.

Two properties make the relaxation checkable rather than a matter of trust. It \textbf{fails
closed}: the kernel is removed by stripping an exact expected prefix, and where that prefix does
not match --- as on a non-Linux host --- the full string is returned and the key compares
everything it compared before, so an unrecognised format cannot silently widen the gate. And the
kernel remains \textbf{stamped per artifact}, so a reader can enumerate exactly which kernels ran
and test any result against them. The change was made before any unit was trained at the pinned
budget, which was mechanically checkable at the time: no evaluation artifact then carried the
pinned step count. That statement is dated rather than standing --- the run has since begun, and
three units at the pinned budget now exist.
% artifact: src/trikaal/utils/provenance.py :: PROVENANCE_IDENTITY_KEYS (16 keys, `platform_abi`
%   at position 12, `platform` absent from the comparison set); platform_abi() strips the exact
%   "{system}-{release}-" prefix and returns the FULL string otherwise; run_provenance() stamps
%   both. Verified at entry-time: the 7 literal rented kernels in
%   tests/run/test_platform_abi_split.py collapse to x86_64-with-glibc2.35; glibc2.31 and aarch64
%   still separate; macOS returns the full string. 40 tests passed across that file and
%   tests/run/test_fanout_refusal.py.
% artifact: docs/m6_fanout_runbook.md §0 table — 1 box 270.4 h compute / 270.7 billed / $79--$108;
%   5 boxes 54.1 h compute / 271.8 billed / $79--$109. Delta +1.1 GPU-h, about +$1.
% artifact: docs/m6_prereg.md §7 v1.6.29 — the 14-box measurement, the attribution sequence, and
%   the correction to the briefed "$96 -> $121" cost figure ($96 appears in no artifact).
% artifact: AS OF 2026-08-11, zero eval artifacts under runs_cloud/ carried
%   provenance.steps_stage2 == 26003. AS OF 2026-08-12 two do (results/r0 cell1_seed0,
%   results/r2 cell1_seed4) — the run has started. The claim above is dated, not standing;
%   §7 v1.6.30 records the change and its consequence for the timing claims. The 23
%   that exist are toy-rehearsal (15), CUDA-validation probe (7) and P2 (1), all at other budgets.
...
five seeds, the flat headline cost of $0.0030$, the evaluation sequence length, the backbone
parameter count of $21{,}301{,}248$ excluding the MTP heads, the $\hat{\mu}$ estimator, the
...
\textbf{That property is weaker than it sounds, and we state the weaker version because the
stronger one was false here.} Loading from a receipt prevents a figure from contradicting the
receipt it reads. It does not prevent a figure from reading the \emph{wrong} receipt, or from
reading no receipt at all for a panel whose result now exists --- and that is exactly what
happened to \cref{fig:legibility}, this paper's primary figure. Its right-hand panel rendered six
empty slots watermarked ``awaiting the run'' for every draft after the gate had fired, because the
generator loaded four fixture-era receipts and contained no reference to the gate's own stop
receipt. Nothing failed; there was nothing to fail. Two figures now carry an explicit check ---
\cref{fig:legibility} asserts its drawn values against the stop receipt, and \cref{fig:design}
asserts its parameter counts against the checkpoints --- and each check has been observed to fail
under a deliberately wrong constant. The general claim is therefore: a figure that
\emph{contradicts} the artifact it loads fails to build, and where a figure could silently omit an
artifact instead, we have added the check rather than assumed the property.
