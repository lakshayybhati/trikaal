...
\toprule
seed & lean & model & bias only & $\Delta$ & $\Delta$ 95\% low \\
\midrule
0 & short, $67.7\%$ & $+1.2665$ & $-0.7607$ & $+2.0271$ & $-0.5530$ \\
2 & short, $65.5\%$ & $+2.1715$ & $-1.2640$ & $+3.4355$ & $\mathbf{+1.5574}$ \\
4 & long, $92.7\%$ & $+1.1443$ & $+0.4282$ & $+0.7161$ & $-0.2410$ \\
\bottomrule
\end{tabular}

\vspace{0.4em}
{\footnotesize\emph{Gross annualized information ratios at $h=15$ on the headline money grid, cell 1
only. \textbf{The benchmark averages over every symbol carrying a valid decision, not over the names
each model actually traded: those positions are not persisted, and the direction of that difference
is unknown and unbounded.} The exact version was authorized and measured infeasible --- see below.}}
\end{minipage}
\end{center}
...
\textbf{On the third it is live, and we state it as such.} Seed 4 is long on $92.7\%$ of its
decisions in a rising sample; holding long reproduces $37.4\%$ of its gross information ratio, and
the $95\%$ interval on the remainder includes zero. \textbf{We cannot exclude that most of seed 4's
gross edge is drift.}

\textbf{What survives, stated at the width the evidence supports:} directional bias times sample
drift does not explain the gross performance of two of three seeds, where that bias would have lost
money; on the third it accounts for $37\%$ and the remainder is not distinguishable from zero; and
\emph{one} of three seeds demonstrates timing content at $95\%$. The claim is not that these models
have predictive skill. It is that on two of three seeds the cheapest deflationary explanation is
refuted, and on one of three the effect clears zero against it.
...
sixfold, on an interval that already straddles zero. A GPU pass would cost about \$$3.57$ per seed
at the measured marginal rate and was not run, economics spend being deferred. We report the price
so the omission is costed rather than assumed impossible.
