...
direction of that difference is unknown and unbounded.} \textbf{Seed 4's $\Delta$ is $t = 1.70$: the
pre-registered percentile interval clears zero at $+0.0454$, a symmetric normal interval would not
($-0.1545$).} Seed 4's model row is the value recomputed during the exact run, $3.5\times10^{-4}$
from the banked one.}}
\end{minipage}
\end{center}
...
%   t = 0.9972.../0.5876... = 1.697; symmetric normal lower = delta - 1.96*se = -0.1545.
...
\textbf{That margin is thin and we report the statistic that argues against us before the one that
argues for us.} The $\Delta$ is $t = 1.70$. Our pre-registered interval is a percentile bootstrap
and it clears zero at $+0.0454$; a symmetric normal interval at the same standard error would not,
reaching $-0.1545$. The result is ``excludes zero'' on the pre-registered instrument and would not
survive a different, equally defensible one. Both directions of this outcome were written down
before the run, so the reading was not chosen after the number: the pre-declaration specified what
we would conclude if the interval excluded zero, if it straddled it, and if it came back negative.
