...
%   123 inline `% artifact:` comments across the section files and main.tex
%   44 distinct artifact paths referenced
...
artifact it came from --- $123$ such comments across $44$ distinct artifact paths, stripped from
...
per-bar legibility, original interface & $0.5135$ & measured & id-visibility probe \\
chance rate & $0.50$ & pin & probe construction \\
per-bar legibility, re-specified & $0.9047$ & measured & $\lambda$-search, seed mean \\
...
DSR trial count & $60$ & pin & conformance \\
bits-per-bar gap, FSQ vs BSQ & $0.058$ & derived & vocabulary arithmetic \\
parameter gap & $69{,}632$ ($0.327\%$) & measured & realized counts \\
...
filter activity on real data & --- & \textbf{pending} & no artifact records it \\
\bottomrule
\end{tabular}
\end{center}

The last two rows are the ones a reader should carry forward. The capacity handicap is disclosed
and unquantified --- no cell in the pre-registration brackets it. And no eval artifact in this
project records decision activity on real bars, so whether the execution filter binds there is
unmeasured rather than answered elsewhere.
...
amendment log & $41$ revision tags & measured & pre-registration \\
...
the verdict manifest, named beside the slot in the source, so that filling it is a mechanical
substitution rather than a judgement. The three figures in that section are drawn on placeholder
...
result. \Cref{sec:res-interpretation} carries both interpretation paragraphs, written and dated
before the run.
