...
amendment log & $52$ revision tags & measured & pre-registration \\
anchor causal sweep & $420$ of $420$ & measured & Gate-A manifest \\
test suite & $911$ collected & measured & \texttt{pytest -{}-collect-only} \\
\bottomrule
\end{tabularx}
\end{center}
...
Appendix~F.1 records that the amendment log carries $52$ distinct revision tags across
$49$ top-level entries, three tags --- v1.0, v1.6.24 and v1.6.27 --- appearing as in-body
...
that section, \emph{two} --- \cref{fig:cost,fig:seeds} --- are drawn on placeholder data that is
implausible by construction, one coincident curve at magnitudes two orders above anything the
design can produce and five cells centred on zero, so that a cropped panel cannot be mistaken for
a result; they remain watermarked. The third, \cref{fig:verdict}, is a schematic of the decision
