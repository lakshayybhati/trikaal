...
%   §3  Mechanism ........ drafted, every number traced to a committed artifact
%   §1  Introduction ..... title / abstract / contributions drafted PROVISIONALLY; body pending
%   §2, §4–§8 ............ structural stubs, drafted in the order §4, §5, §7, §8, §2, §1
%   Any \TBD{...} is a quantity that does not exist until the pre-registered run completes.
%   No \TBD may be filled by estimate.
% =============================================================================================
...
\label{sec:intro}
% STUB — funnel prose pending. Contributions and Figure 1 drafted for page-1 review.
...
\begin{figure}[t]
  ...
  are identical between the two panels. \textbf{Below:} the same fixture, the same training budget
  and the same $1.151$-nat signal planted in the microstructure channel, before and after --- the
  planted arm ends $0.0134$ nats \emph{worse} than its matched control on the left, and $1.3381$
  nats better on the right.}
  \label{fig:mechanism}
\end{figure}
...
\label{sec:related}
\TBD{§2 --- four clusters: K-line foundation models; discrete tokenization (VQ/FSQ/BSQ);
microstructure prediction with the TFI-vs-OFI distinction; evaluation discipline. Drafted after
§4, §5, §7, §8.}
...
\section{Data}
\label{sec:data}
\TBD{§5 --- prose pending; the coverage figure is final and enumerated from the lake.}
...
\section{Results}
\label{sec:results}
\TBD{§6 --- structured stubs only. No number below exists until the pre-registered run completes; the three figures are built against the artifact schema and are watermarked until then. Both interpretation paragraphs are pre-written and dated.}
...
\section{Reproducibility and verification}
\label{app:repro}
\TBD{pinned decision surface; content hashes; environment lockfiles; amendment log.}

\section{Claims audit}
\label{app:claims}
\TBD{measured-versus-estimated table.}
